...
class TournamentParameters:
    def __init__(self, rounds, strength_change, logistics_mu, logistics_sigma):
        self.rounds = rounds
...
def simulate_tournament(team_strengths):
    ...
    for rnd in parameters.rounds[:-1]:
...
final_results_df['Team1_name'] = final_results_df['Team1'].map(teams.set_index('Team')['Team Name'])
final_results_df['Team2_name'] = final_results_df['Team2'].map(teams.set_index('Team')['Team Name'])
final_results_df['Winner_name'] = final_results_df['Winner'].map(teams.set_index('Team')['Team Name'])
final_results_tournament = final_results_df[['Team1_name', 'Strength1', 'Team2_name', 'Strength2', 'Winner_name', 'NewStrength']]
final_results_tournament = final_results_tournament.reset_index(drop=True)

final_results_tournament.to_csv("/home/azureuser/cloudfiles/code/giggles/final_results_tournament.csv", index=False)
